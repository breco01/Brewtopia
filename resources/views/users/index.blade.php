                            ...
                            @foreach ($users as $user)
                                <tr class="border-b">
                                    <td class="px-4 py-2">
                                        ...
                                        @else
                                            <img src="https://www.gravatar.com/avatar/{{ md5(strtolower($user->email)) }}"
                                                alt="Profielfoto" class="w-10 h-10 rounded-full">
                                        @endif
                                    </td>
                                    ...
                                    <td class="px-4 py-2">
                                        ...
                                        <form action="{{ route('users.updateAdminStatus', $user) }}" method="POST"
                                            class="inline-block">
                                            @csrf
                                            @method('PUT')
                                            <select name="is_admin" id="is_admin-{{ $user->id }}" class="form-select">
                                                <option value="1" {{ $user->is_admin ? 'selected' : '' }}>Admin</option>
                                                <option value="0" {{ !$user->is_admin ? 'selected' : '' }}>Geen Admin
                                                </option>
                                            </select>
                                            <button type="submit"
                                                class="bg-yellow-500 text-white py-2 px-4 rounded-full hover:bg-yellow-600 mt-2">
                                                Opslaan
                                            </button>
                                        </form>
                                    </td>
